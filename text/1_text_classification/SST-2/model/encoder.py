...
A recurrent layer walks the sentence one token at a time, carrying a hidden
...
additive highway, which is why LSTM/GRU beat the Elman RNN on any sentence
where the verdict and its target are far apart ("the plot, which meanders for
two hours and never justifies its cast, is *dull*"). config.CELL picks which
one runs, everything else stays identical -- see the README comparison.
...
Two mechanics implemented here that are specific to variable-length text:
  ...
  the sentence ended, so the "final" state would be the state after k padding
  steps -- a different (and wrong) vector for every sentence length.
  ...
  BIDIRECTIONALITY. Sentiment cues sit anywhere in the sentence, and a forward
  RNN's representation of token 3 knows nothing about token 20. A second layer
  reading right-to-left fixes that; the two directions are concatenated, which
  is why the head sees 2*H features.
"""
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, lengths: torch.Tensor):
        ...
        Output:
            ...
            final:   sentence-level state [B, H*dirs] -- the LAST layer's
