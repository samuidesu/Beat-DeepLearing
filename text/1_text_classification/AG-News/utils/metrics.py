...
AG News' standard metric is plain ACCURACY, and here that is unusually safe to
trust: the corpus is EXACTLY balanced (25% per class in train, val and test
alike), so accuracy cannot be inflated by a majority-class shortcut and the
chance baseline is a clean 0.25.
...
With four balanced classes, accuracy and macro-F1 track each other closely, so
the interesting content is not the headline but the OFF-DIAGONAL structure. A
topic classifier's errors are not uniform: Business and Sci/Tech overlap
heavily (a story about a chip-maker's earnings is genuinely both), while
Sports is nearly disjoint from everything. The confusion matrix is where that
shows up, and it is the reason print_report always prints the full table
rather than just the two summary numbers.
"""
...
if __name__ == "__main__":
    # 8 samples, 4 classes, 2 of each. Class 0 and 3 are confused with each
    # other (the Business/Sci-Tech pattern in miniature); 1 and 2 are perfect.
    gt = torch.tensor([0, 0, 1, 1, 2, 2, 3, 3])
    pred = torch.tensor([0, 3, 1, 1, 2, 2, 0, 3])

    cm = ConfusionMatrix(num_classes=4)
    cm.update(pred, gt)
    print_report(cm.compute(), class_names=["c0", "c1", "c2", "c3"])

    # By hand: 6 of 8 correct -> accuracy 0.75
    # c0: TP=1 FP=1 FN=1 -> P=0.5 R=0.5 F1=0.5
    # c1: TP=2 FP=0 FN=0 -> P=R=F1=1.0
    # c2: TP=2 FP=0 FN=0 -> P=R=F1=1.0
    # c3: TP=1 FP=1 FN=1 -> P=0.5 R=0.5 F1=0.5
    # macro F1 = (0.5 + 1 + 1 + 0.5) / 4 = 0.75
    print("\nexpected accuracy 0.7500, macro_f1 0.7500")
    print("expected c0 and c3 P/R = 0.5000 / 0.5000, c1 and c2 = 1.0000")
