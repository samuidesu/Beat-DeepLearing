...
copy lived next to the one dataset. Copied unchanged from the SST-2 project
except for extract_tar at the bottom -- AG News ships as a .tgz, GloVe as a
.zip.
"""
...
def extract_tar(archive: str, dest_dir: str):
    """Extract a .tar / .tgz / .tar.gz archive into `dest_dir`.

    The zip counterpart above can extract selected members because GloVe's zip
    holds four files we do not all want. AG News' tgz holds exactly the three
    small files we do want (train.csv, test.csv, classes.txt), so there is
    nothing to select and the signature stays simpler.
