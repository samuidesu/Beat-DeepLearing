...
is exactly why the two-stage protocol transfers from the CNN projects:
    ...
    stage 2  unfreeze() -> the table finetunes at a much smaller LR, so
                           topic-specific nuance can be learned without
                           destroying the general geometry ("bond" is a
                           Business word here far more often than a
                           chemistry or a Bond-film word)
...
Note the scale of this layer on AG News: ~45.6k vocabulary rows x 100 dims is
4.6M parameters, which is MORE than the BiLSTM encoder above it. Most of the
model is a dictionary, and stage 1 exists precisely so that dictionary is not
being rewritten by gradients before the encoder knows how to read it.
"""
