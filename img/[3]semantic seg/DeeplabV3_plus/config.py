...
# This project is DeepLab-v3+: trainV3.py writes here by default.
OUTPUT_DIR_V3PLUS = os.path.join(PROJECT_ROOT, "outputsv3plus")
...
ASPP_V3_RATES = (3, 6, 9)
ASPP_V3_HIDDEN = 256   # per-branch feature width in the ASPP encoder

# --- DeepLab-v3+ decoder (used by trainV3.py) ---
# Channels the decoder reduces the stride-4 low-level feature (c2, 64ch) to
# before fusing it with the 256-ch encoder output. 48 is the paper's default:
# small enough that the low-level detail doesn't overwhelm the semantics.
LOW_LEVEL_PROJ = 48
