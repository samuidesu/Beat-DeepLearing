...
def _set_conv_dilation(
    ...
    if conv.kernel_size != (3, 3):
        raise ValueError(
            f"expected 3x3 convolution, " f"got kernel_size={conv.kernel_size}"
        )
...
def _remove_stage_downsampling(
    ...
    if first_block.downsample is None:
        raise ValueError("expected first block to contain a downsample path")
...
def _replace_stage_stride_with_progressive_dilation(
    ...
    if first_block_dilation < 1:
        raise ValueError("first_block_dilation must be positive")

    if later_block_dilation < 1:
        raise ValueError("later_block_dilation must be positive")
...
class ResNetBackbone(nn.Module):
    ...
    def __init__(
        ...
        if arch not in _RESNET_ARCHS:
            raise ValueError(
                f"unsupported backbone {arch!r}; " f"choose from {tuple(_RESNET_ARCHS)}"
            )

        constructor, weights_enum, final_channels = _RESNET_ARCHS[arch]
