"""Training entry point for HRNetV2-W32 semantic segmentation on PASCAL VOC 2012.

The HEAD is selectable and each head owns its output folder:
    --head simple  -> HRNetV2 concat head        -> outputs_simple/
    --head ocr     -> OCR head (+ aux loss 0.4)  -> outputs_ocr/
so simple-vs-OCR is a clean single-variable comparison sharing everything else
(backbone, data, loss contract, schedule, eval protocol).

Two-stage finetuning (same shape as the DeepLab experiments):
    Stage 1 - freeze only the LOW backbone (stem + layer1); the parallel-
              branch stages (whose cross-resolution fusion convs must adapt to
              segmentation anyway) train with the head, at a smaller LR
              (pretrained weights ADAPTING vs. random weights LEARNING).
    Stage 2 - unfreeze the ENTIRE backbone and finetune with LAYERED
              learning rates: low backbone slowest, stages middle, head
              fastest.

NOTE on pretrained weights: torchvision has no HRNet, so the backbone loads
config.PRETRAINED_BACKBONE (hrnetv2_w32_imagenet_pretrained.pth) if the file
exists. Without it training runs FROM SCRATCH -- then skip stage 1
(--epochs-stage1 0): freezing RANDOM low layers helps nobody.
...
curve plotting -- deliberately parallel to the DeepLab train.py.

Usage:
    python train.py                       # simple head, config.py defaults
    python train.py --head ocr            # OCR head -> outputs_ocr/
...
from losses.hrnet_loss import HRNetLoss
...
def collect_run_meta(args, device, train_loader, val_loader, *, model_name, neck):
    ...
    return {
        ...
        "model_cfg": {
            ...
            "hrnet_channels": list(config.HRNET_CHANNELS),
            "hrnet_num_blocks": config.HRNET_NUM_BLOCKS,
            "hrnet_num_modules": list(config.HRNET_NUM_MODULES),
...
def plot_curves(history, output_dir):
    ...
    plt.ylabel("metric")
    plt.ylim(0, 1)
    plt.title("HRNet val mIoU / pixel accuracy")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
# -----------------------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Train HRNetV2-W32 on PASCAL VOC 2012 segmentation")
    ...
    p.add_argument("--epochs-stage1", type=int, default=config.STAGE1_EPOCHS,
                   help="stage-1 epochs; 0 skips stage 1 (use 0 when training "
                        "from scratch, i.e. no pretrained backbone file)")
    ...
    p.add_argument("--head", choices=["simple", "ocr"], default=config.HEAD,
                   help="segmentation head: simple (HRNetV2 concat head, -> "
                        "outputs_simple/) or ocr (OCR head + aux loss, -> "
                        "outputs_ocr/)")
    p.add_argument("--output-dir", default=None,
                   help="output folder name/path. Default: outputs_simple/ or "
                        "outputs_ocr/, per --head. A bare name is placed under "
                        "the project root; an absolute path is used as-is.")
    return p.parse_args()
...
def main():
    ...
    # Output folder: one per head (outputs_simple/ vs outputs_ocr/) so the two
    # experiments never overwrite each other; --output-dir overrides.
    if args.output_dir:
        output_dir = (args.output_dir if os.path.isabs(args.output_dir)
                      else os.path.join(config.PROJECT_ROOT, args.output_dir))
    else:
        output_dir = config.output_dir_for_head(args.head)
    os.makedirs(output_dir, exist_ok=True)
    print(f"Device: {device}")
    print(f"Data root: {config.DATA_ROOT}")
    print(f"Output dir: {output_dir}  (HRNetV2-W32, {args.head} head)")
    ...
    # HRNetLoss handles both heads: plain CE for simple, CE + aux-weighted CE
    # for the OCR head's (main, aux) training output.
    criterion = HRNetLoss(ignore_index=config.IGNORE_INDEX,
                          aux_weight=config.AUX_LOSS_WEIGHT).to(device)

    # From-scratch guard: stage 1 freezes the low backbone, which only makes
    # sense when those weights are pretrained. Warn (don't override) so the
    # run is still reproducible from the command line alone.
    if not model.backbone.pretrained_loaded and args.epochs_stage1 > 0:
        print("[HRNet] WARNING: no pretrained backbone was loaded but stage 1 "
              "will freeze the (random) stem+layer1. Consider "
              "--epochs-stage1 0 for from-scratch training.")

    history = []
    best = {"miou": -1.0, "epoch": -1}

    # ---- Stage 1: freeze LOW backbone, train branch stages + head ----
    if args.epochs_stage1 > 0:
        print("\n=== Stage 1: freeze stem + layer1, "
              "train stages 2-4 + head ===")
        model.freeze_backbone_low()
        print(f"Trainable params: {count_trainable(model):.2f}M")
        # Two LR tiers: from-scratch head fast, pretrained branch stages
        # slower. backbone_low is frozen -> its tier is skipped.
    ...
    head_meta = {"head": args.head,
                 "pretrained_backbone_loaded": model.backbone.pretrained_loaded}
    if args.head == "ocr":
        head_meta.update({"ocr_mid_channels": config.OCR_MID_CHANNELS,
                          "ocr_key_channels": config.OCR_KEY_CHANNELS,
                          "aux_loss_weight": config.AUX_LOSS_WEIGHT})
    meta = collect_run_meta(
        args, device, train_loader, val_loader,
        model_name=f"HRNetV2-W32 ({args.head} head)",
        neck=head_meta)
